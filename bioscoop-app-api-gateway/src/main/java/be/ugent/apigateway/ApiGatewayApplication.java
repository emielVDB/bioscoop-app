...
public class ApiGatewayApplication implements WebFluxConfigurer {
    ...
    public RouteLocator customRouteLocator(RouteLocatorBuilder builder) {
        return builder.routes()
                ...
                // hall event service routes
                .route(r -> r.host("**").and().path("/eventhall/**").uri("http://localhost:2221"))
                ...
                .route(r -> r.host("**").and().path("/advertisement/**").uri("http://localhost:2226"))
                // catering service routes
                .route(r -> r.host("**").and().path("/catering/**").uri("http://localhost:2227"))
                // statistics service routes
                .route(r -> r.host("**").and().path("/statistics/**").uri("http://localhost:2228"))
